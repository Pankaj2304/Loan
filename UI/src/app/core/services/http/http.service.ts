...
export class HttpService {
  constructor(private http: HttpClient, private loader: LoaderService, private message: MessageService) {}

  /******************** HTTP Requests ********************/
  getData(url: string, params?: { [x: string]: any }): Observable<any> {
    const _params = params ? { params: this.appendParams(params) } : {};
    this.loader.show();
    return this.http.get<any>(environment.BASE_URL + url, _params).pipe(
      map((response: any) => {
        this.loader.hide();
  ...
  putData(url: string, data: { [x: string]: any }): Observable<any> {
    this.loader.show();
    return this.http.put<HttpClient>(environment.BASE_URL + url, data).pipe(
      map((response: any) => {
        this.loader.hide();
  ...
  deleteData(url: string, id: string): Observable<any> {
    this.loader.show();
    return this.http
      .delete<HttpClient>(environment.BASE_URL + url)
      .pipe(
        map((response: any) => {
          this.loader.hide();
  ...
  postData(
    ...
    return this.http.post<HttpClient>(environment.BASE_URL + url, data).pipe(
      map((response: any) => {
